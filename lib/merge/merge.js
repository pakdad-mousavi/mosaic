...
export const main = async ({ files, dir, columns, size, recursive }) => {
  const images = await loadImages({ files, dir, recursive });
  const sheet = await useSquaredMode(images, columns, size);
};

export const validateParams = ({ files, dir, columns, size, recursive }) => {
  ...
  // Ensure size is either null or an integer
