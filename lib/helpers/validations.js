...
export const validateSharedOptions = ({ files, dir, recursive, shuffle, gap, canvasColor, output }) => {
  ...
  const formattedParams = {
    ...
    shuffle,
    gap: Number(gap),
    canvasColor: canvasColor === 'transparent' ? '00000000' : canvasColor,
    output,
  };
...
export const validateSquareOptions = ({ fitMode, imageSize, paddingColor, columns, caption, captionColor }) => {
  ...
  const formattedParams = {
    ...
    paddingColor: paddingColor === 'transparent' ? '00000000' : paddingColor,
    columns: Number(columns),
    caption,
    captionColor: captionColor === 'transparent' ? '00000000' : captionColor,
  };

  return formattedParams;
};

export const validateMasonryOptions = ({ rowHeight, columnWidth, rows, columns, orientation, hAlign, vAlign }) => {
  ...
  if (rows && (isNaN(rows) || !Number.isInteger(Number(rows)) || Number(rows) < 1)) {
    throw new Error('--rows must be a positive integer.');
  }

  if (columns && (isNaN(columns) || !Number.isInteger(Number(columns)) || Number(columns) < 1)) {
    throw new Error('--columns must be a positive integer.');
  }
