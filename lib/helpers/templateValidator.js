...
export const validateTemplate = (json) => {
  ...
  if (Math.floor((json.canvas.width - json.canvas.gap * 2) / json.canvas.columns) === 0) {
    throw new Error(`Columns are too thin. Increase canvas width or reduce number of columns.`);
  }

  // Ensure rows are thick enough
  if (Math.floor((json.canvas.height - json.canvas.gap * 2) / json.canvas.rows) === 0) {
