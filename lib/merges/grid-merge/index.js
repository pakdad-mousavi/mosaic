...
export const gridMerge = async (files, images, validatedParams) => {
  // Destructure params
  const { aspectRatio, imageWidth, columns, gap, arc, canvasColor, caption, captionColor, maxCaptionSize } = validatedParams;
  ...
  const roundedImages = await roundImages(resizedImages, { width, height, arc });
